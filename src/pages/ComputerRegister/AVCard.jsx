...
import {
  ...
  getAvequipmentById,
  updateAvequipment,
} from "../../data/Avequipment/Avequipment.repository.js";
...
export default function AVCard() {
  const navigate = useNavigate();
  const { id } = useParams();
  const locationRouter = useLocation();
  const passedOrgId = locationRouter.state?.orgId || "";
  ...
  useEffect(() => {
    if (id && id !== "new") {
      ...
      getAvequipmentById(id)
        ...
        .catch(() => setError("Audit Trail Error: Could not retrieve AV equipment record."))
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!String(form.equipmentType || "").trim()) throw new Error("Compliance: Equipment Type is mandatory.");
      ...
      const payload = {
        ...
        connectionType: String(form.connectionType || "").trim(),
        ...
        previousOwner: String(form.previousOwner || "").trim() || null,
        purchaseAmount,
      ...
      if (isCreate) {
        await createAvequipment(payload);
        setToast({ show: true, message: "Asset saved successfully.", type: "success" });
      } else {
        await updateAvequipment(id, payload);
        setToast({ show: true, message: "Asset saved successfully.", type: "success" });
      }
    ...
    <>
      <div className="sideBlock">
        ...
        <div className="summary">
          ...
          <div className="summaryRow">
            <div className="summaryLabel">Equipment</div>
            <div className="summaryValue">{form.equipmentType || "-"}</div>
          </div>
          <div className="summaryRow">
  ...
  return (
    <DetailsLayout
      title={isCreate ? "Register New AV Asset" : `Edit: ${form.equipmentType || id}`}
            ...
            {!makeManual ? (
              <div style={{ flex: 1, position: "relative" }}>
                <Zbot_Fields
                  ...
                  placeholder="Search Brands (Sony, Epson)..."
                  className="value"
                />
